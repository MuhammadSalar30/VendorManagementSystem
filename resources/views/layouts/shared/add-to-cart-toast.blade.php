...
.animate-slide-in { animation: slideIn .25s ease-out; }
 ...
 <script>
  function openCartDrawer() {
    ...
    if (!drawer || !body) return;
    drawer.classList.remove('hidden');
    fetch('/cart/side', { cache: 'no-store' })
      .then(r => r.text())
      .then(html => {
        body.innerHTML = html;
        if (window.lucide && typeof window.lucide.createIcons === 'function') { window.lucide.createIcons(); }
...
function updateCartBadge(count) {
    ...
    const badge = document.querySelectorAll('.cart-badge, #cartCount, .navbar-cart-count');
    badge.forEach(el => {
        el.textContent = count;
        el.style.display = count > 0 ? 'inline-block' : 'none';
    });
...
function recalcSideCartCount() {
    let count = 0;
    // If logged in, fetch from server (optional: you can use AJAX to get latest count)
    if (window.isLoggedIn) {
        // Count from DOM (side cart)
        document.querySelectorAll('#cartDrawer [data-cart-item-id]').forEach(row => {
        ...
        cart.forEach(item => { count += item.quantity || 0; });
    }
...
function updateSideQuantity(cartItemId, change) {
  ...
  if (window.isLoggedIn) {
    ...
    // Guest user → update local cart (example using localStorage)
    let cart = JSON.parse(localStorage.getItem('guestCart') || '{}');
    if (cart[cartItemId]) {
      cart[cartItemId].quantity = next;
    }
...
function recalcSideSubtotal() {
  ...
  const subEl = document.getElementById('side-subtotal');
  if (subEl) subEl.textContent = `Rs ${subtotal.toLocaleString(undefined,{minimumFractionDigits:0,maximumFractionDigits:0})}`;
  loadCartCount();
}
...
function removeFromSideCart(cartItemId) {
  if (window.isLoggedIn) {
    ...
    // Guest → delete from localStorage/session
    let cart = JSON.parse(localStorage.getItem('guestCart') || '{}');
    if (cart[cartItemId]) {
      delete cart[cartItemId];
      localStorage.setItem('guestCart', JSON.stringify(cart));
    ...
    // Recalculate totals + update cart badge
    recalcSideSubtotal();
    loadCartCount();
    refreshCartCounts();
  }
...
window.bumpCartCount = function(amount) {
    ...
    // Ensure the bottom cart bar is visible if the count is greater than 0
    const btn = document.getElementById('bottomCartButton');
    if (btn && parseInt(document.querySelector('.cart-count').textContent) > 0) {
...
window.showAddToCartToast = function() {
    showBottomCartBar();
    // Show temporary notification; rely on server count refresh to avoid double increments
    const existing = document.getElementById('inlineCartNotice');
    if (existing) existing.remove();
    const notice = document.createElement('div');
    notice.id = 'inlineCartNotice';
